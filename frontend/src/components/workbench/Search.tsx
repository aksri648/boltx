...
async function performTextSearch(
  ...
  const { exitCode, result } = await sandboxApi.exec(
    `grep -rn "${escapedQuery}" ${WORK_DIR} --include='*.*' --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=dist --exclude-dir=.cache --exclude='package-lock.json' --exclude='*.lock' 2>/dev/null | head -500`,
  );
  ...
  for (const line of lines) {
    ...
    batch.push({
      ...
      lineNumber,
      previewText: content,
      matchCharStart: queryIdx >= 0 ? queryIdx : 0,
      matchCharEnd: queryIdx >= 0 ? queryIdx + query.length : content.length,
    });
  }
...
export function Search() {
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      await performTextSearch(query, progressHandler);
    } catch (error) {
