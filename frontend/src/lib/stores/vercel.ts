...
export async function autoConnectVercel() {
  ...
  try {
    ...
    const proxyResponse = await fetch('/api/vercel-proxy/user', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ token: envToken }),
    });

    console.log('Vercel proxy response status:', proxyResponse.status);

    if (!proxyResponse.ok) {
      const err = await proxyResponse.json().catch(() => ({ error: 'Unknown error' }));
      throw new Error(`Vercel API error: ${err.error}`);
    }

    const userData = (await proxyResponse.json()) as any;
      ...
      projects.map(async (project: any) => {
        try {
          const deploymentsResponse = await fetch('/api/vercel-proxy/deployments', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token, projectId: project.id, limit: 1 }),
          });
