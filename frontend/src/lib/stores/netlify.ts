...
export async function initializeNetlifyConnection() {
  ...
  try {
    ...
    const proxyResponse = await fetch('/api/netlify-proxy/user', {
      method: 'POST',
    ...
    if (!proxyResponse.ok) {
      const err = await proxyResponse.json().catch(() => ({ error: 'Unknown error' }));
      throw new Error(`Failed to connect to Netlify: ${err.error}`);
    }
...
export async function fetchNetlifyStats(token: string) {
  try {
    ...
    const proxyResponse = await fetch('/api/netlify-proxy/sites', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!proxyResponse.ok) {
      throw new Error(`Failed to fetch sites: ${proxyResponse.status}`);
    }

    const sites = (await proxyResponse.json()) as any;
