...
const _dbPromise: Promise<IDBDatabase | undefined> = (async () => {
  ...
  const restoreSnapshot = useCallback(async (id: string, snapshot?: Snapshot) => {
    ...
    for (const [key, value] of Object.entries(validSnapshot.files)) {
      ...
      if (value?.type === 'folder') {
        await sandboxApi.mkdir(filePath);
      }
    }
    ...
    for (const [key, value] of Object.entries(validSnapshot.files)) {
      if (value?.type === 'file') {
        ...
        await sandboxApi.writeFile(filePath, value.content);
      }
    }
  }, []);
